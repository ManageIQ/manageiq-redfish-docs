...
the physical systems into a selected Operating System. The process relies on
the Preboot Execution Environment (PXE) functionality offered by the services
in the data center to the physical servers, which, in turn, are capable of
loading the PXE automatically off of the network. Upon confirmation of the
Physical Server Provisioning request, the {redfish-provider} configures the
PXE server with the network boot parameters specified in the request for the
servers defined in the request. It then restarts these servers into one-time
network boot, initiating the provisioning.

==== Configuring PXE

In {product-title}, a PXE server represents a collection of services and files
that, all together, enable booting into Operating Systems from the network:

* The {product-title} needs to be able to connect to the PXE server for both
  reading and writing using either NFS or SAMBA protocol. This is also called
  the depot type.
* The physical servers' NICs that are connected to the network boot network
  require a TFTP access to the initial executable file containing the PXE
  itself, and the HTTP access to the Operating Systems' files.

The process of configuring the PXE Servers is similar to that explained in the
*PXE Provisioning* section of the *Provisioning Virtual Machines and Instances*
chapter.
...
* *URI* (Network File System depots only): the access point to the Network
  File System mount.
* *Access URL*: the address of the HTTP server.
* *PXE Directory*: the path, relative to the depot type's mount point, to the
  PXE menu folder.
* *Customization Directory*: relative path to the customization templates files.
  This directory will be appended to the _Access URL_, resulting in a prefix
  for the operating systems to obtain the customization templates.
* *Filename*: relative path to the default boot menu file. {product-appliance}
  will read this file to retrieve the list of available operating systems on the
  PXE server.

We click *Add* to add the new server. Then we perform *Configuration -> Refresh
Relationships* and, after a few seconds, refresh the browser.
...
In the *Type* field, leave the value *<Any>*. We confirm the new system image
